...
<section id="content-section">
    <div class="page-container">
        ...
        <div class="upcoming-page gerenric-padding">
            <form method="GET" action="{{url('user')}}">

                <div class="gerenric-search-bar">
                    <div class="search-bar">
                        <div class="search-block">
                            <input type="text" name="search" class="search-input" value="{{isset($fields['search']) && !is_null($fields['search']) ? $fields['search'] : '' }}" placeholder="Search">
                    ...
                    <div class="filter-block">
                        ...
                        <div class="filter-dropbox">
                            ...
                            <div class="gerenric-form">
                                <ul>
                                    <li><label class="form-label">Booking Number</label><input name="booking_number" type="text" class="form-control" placeholder="Booking Number" value="{{isset($fields['booking_number']) && !is_null($fields['booking_number']) ? $fields['booking_number'] : '' }}"></li>
                                    <li><label class="form-label">Booking Date</label><input name="booking_date" type="date" class="form-control" placeholder="Booking Date" value="{{isset($fields['booking_date']) && !is_null($fields['booking_date']) ? $fields['booking_date'] : '' }}"></li>
            ...
            <div class="gerenric-table-desktop column-10">
                ...
                <div class="table-row">
                    ...
                    <div class="table-col">
                        <div class="passenger-dt">
                            <div class="passenger-info">
                                <div class="passenger-col ">
                                    {{ ucfirst($ride->display_name) }} -
                                    @if($ride->payment_method=='cash')
                                    <img class="ms-2" src="{{asset('images/cash_tb_icon.svg')}}" alt=""> Cash
                                    @elseif($ride->payment_method=='card')
                                    <img class="ms-2" src="{{asset('images/card_icon.svg')}}" alt=""> Card
                                    @endif
                                </div>
                            </div>
                        </div>
                        @if($ride->whatsapp_number)
                        <div class="passenger-col mt-2">
                            <a href="https://wa.me/{{$ride->whatsapp_number}}" target="_blank"> <img width="15px" height="15px" src="{{asset('images/whatsapp_icon_t.svg')}}" alt=""> {{$ride->whatsapp_number}}</a>
                        </div>
                        @else
                        <div class="passenger-whatsapp-button whatsapp mt-2" data-ride_id="{{$ride->id}}" data-bs-toggle="modal" data-bs-target="#whatsapp_popup"><img src="{{asset('images/whatsapp_icon.svg')}}" alt=""> Add Whatsapp</div>
                        @endif
                    </div>
                    <div class="table-col">
                        <div class="ride-type">
                    ...
                    <div class="table-col">{{count($ride->coupons)>0? $ride->price - ($ride->price * $ride->coupons[0]['discount_percentage']) / 100:$ride->price}} AED</div>
                    <div class="table-col">
                    ...
                    <div class="table-col">
                        <div class="reschedule-ride reschedule-ride-btn" data-ride_id="{{$ride->id}}" data-bs-toggle="modal" data-bs-target="#reschedule_popup"><img src="{{asset('images/reschedule_icon.svg')}}" alt="">Reschedule Ride</div>
                        <div class="cancel-ride" data-bs-toggle="modal" data-ride_id="{{$ride->id}}" data-bs-target="#exampleModal_cc"><img src="{{asset('images/cancel_icon.svg')}}" alt="">Cancel Ride</div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="gerenric-table-mobile">
                ...
                <div class="table-box">
                    @if($ride->ridetype->type=='tour')
                    <div class="table-box-main-title"><img src="{{asset('images/city_tour_icon_upcoming.svg')}}" alt=""> CITY TOUR - {{$ride->tour_city}}</div>
                    @endif
                    <div class="client-pick-info">
                        <div class="cp-row">
                            <div class="cp-col">{{ date('D, M d,', strtotime($ride->date_time)) }} {{ date('h:i A', strtotime($ride->ride_time)) }}</div>
                            <div class="cp-col">{{ ucfirst($ride->car) }}</div>
                        </div>
                        <div class="cp-row">
                            <div class="cp-col">
                    ...
                    <div class="pickdrop-location">
                        ...
                        <div class="pick-time">
                            ...
                            <img src="images/time_tb_icon.svg" alt="">
                            {{ date('D, M d, h:i A', strtotime($ride->date_time)) }}
                            @elseif($ride->ridetype->type == 'driver')
                            <img src="images/time_tb_icon.svg" alt="">
                            {{ date('D, M d, h:i A', strtotime($ride->date_time)) }}
                            (
                            ...
                            @else
                            <div class="pt-col">
                                <img src="{{asset('images/kg_tb_icon.svg')}}" alt="">
                                {{$ride->distance}} km
                            </div>
                            <div class="pt-col">
                                <img src="images/time_tb_icon.svg" alt="">
                    ...
                    <div class="action-button">
                        <div class="btn btn-primary reschedule-ride-btn" data-ride_id="{{$ride->id}}" data-bs-toggle="modal" data-bs-target="#reschedule_popup">Reschedule Ride</div>
                        <div class="btn btn-primary cancel-ride" data-ride_id="{{$ride->id}}" data-bs-toggle="modal" data-bs-target="#exampleModal_cc">Cancel Ride</div>
                    </div>
                </div>
                @endforeach
                @endif
    ...
    <form method="POST" id="ride_cancel_form" action="{{url('user/ride/cancel')}}">
        @csrf
        <input name="ride_id" type="hidden" id="ride_id" value="">
    </form>
</section>
<!--CONTENT_SECTION_END-->
            ...
            <div class="whatsapp-popup">
                ...
                <div class="modal-body">
                    <div class="whatsapp-row">
                        ...
                        <div class="whatsapp-field"><input type="text" id="phone" name="whatspp_number" class="form-control" placeholder="+971"></div>
                    </div>
                    <button class="btn btn-primary" id="whatsapp_number">Save</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    $('.ride_date').attr('min', today);
    var ride_cancel = false;
    ...
    $(".cancel-ride").click(function() {
        ride_id = $(this).data("ride_id");
        $("#ride_id").val(ride_id);
        $('#error_text').text('Are You Sure You Want To Cancel The Ride?');
        ride_cancel = true;
    });

    $("#error_btn").click(function() {
        $("#exampleModal_cc").modal('hide');
        if (ride_id != 0 && ride_cancel == true) {
            $("#ride_cancel_form").submit();
        }
    });
    var red_ride = 0;

    $(".reschedule-ride-btn").click(function() {
        red_ride = $(this).data("ride_id");
    });

    $("#reschedule_btn").click(function() {

        var ride_date = $('.ride_date').val();
        var ride_time = $('.ride_time').val();
        if ($('.ride_date').val() === '') {
            $('#error_text').text('Please select date time');
            $('#error_btn').text('Close it!');
            $("#exampleModal_cc").modal('show');
            return;
        }
        $.ajax({
            url: "{{url('user/ride/reschedule')}}",
            data: {
                _token: "{{ csrf_token() }}",
                ride_date: ride_date,
                ride_time: ride_time,
    ...
    $(document).ready(function() {
        ...
        $('.ride_date, .ride_time').on('change', function() {
            validateInitialRideDateTime();
        });

        function validateInitialRideDateTime() {
            var startDate = $('.ride_date').val();
            var startTime = $('.ride_time').val();
            if (!startDate || !startTime) {
                return; // One of the fields is empty, do not proceed
            }

            var startDateTime = new Date(startDate + ' ' + startTime);
            if (!isValidDateTime(startDateTime)) {
                showDateTimeError('The start time should be greater than the current time.');
                $('.ride_date, .ride_time').val(''); // Clear invalid start values
            }
        }

        function isValidDateTime(dateTime) {
            var now = new Date();
            return dateTime > now; // Check if the dateTime is in the future
        }

        function showDateTimeError(errorMessage) {
    ...
    $('#whatsapp_number').click(function(e) {
        e.preventDefault();
        ...
        var url = '{{url("update/user-whatsapp-number ")}}';
        $.ajax({
            url: url,
            type: 'post',
            data: {
