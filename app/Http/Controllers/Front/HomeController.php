...
use App\Mail\RideCancel;
use App\Mail\AdminCancel;
use App\Mail\RideReschedule;
use Carbon\Carbon;
use Stripe\Stripe;
use DateTime;
use NoCaptcha;
...
class HomeController extends Controller
{
    public function index(){
        ...
        return view('front.index',compact('coupons'));
    }
    public function rides_step_one(Request $request){
        ...
        // Validate the Selected Date time and Return Date time 
            ...
            if (!empty($request->rides_return_date)){
                ...
                $dateString = $request->rides_date." ".$request->rides_time;
                $date = Carbon::createFromFormat('D, d M, Y h:i A', $dateString);
                $ride_date_time = $date->format('Y-m-d H:i:s');

                $dateString = $request->rides_return_date." ".$request->rides_return_time;

                $date = Carbon::createFromFormat('D, d M, Y h:i A', $dateString);
                $return_date_time = $date->format('Y-m-d H:i:s');

                if (Carbon::parse($ride_date_time)->greaterThan(Carbon::parse($return_date_time))) {
                   
                    
                    return response()->json(['success' =>false,'message' => 'Return Date should be greater then ride date ']);
                }
        ...
        $is_pickup_airport = isAirportRide($request->source);
        ...
        $session_data = [
            ...
            'is_pickup_airport' => $is_pickup_airport
        ];
    ...
    public function post_step_two(Request $request){
        ...
        if (session("step_one_data")["way"] == "two-way") {
            
            $return_ride_total = $request->return_ride_amount  ;
        }
        ...
        $total_amount_text = $total_ride_amount;

        if ($return_ride_total > 0 && $children_amount > 0) {
            $total_amount_text += $children_amount;
        }

        if ($return_ride_total > 0 && $tip_amount > 0) {

            $total_amount_text += $tip_amount;
        }
        ...
        $data = [
            ...
            'return_amount' => $return_ride_total,
            'discount_amount' => $request->discountamount,
            'note' => $request->note,
            'return_discount' => $request->return_discount,
            'return_discount_amount' => $request->return_discount_amount,
            'onward_price' => currency_format($ride_amount - $return_ride_total),
            'return_text' => currency_format($return_ride_total),
            'children_amount_text' => currency_format($children_amount),
            'children_seat_text' => "(".$request->child_seat.")". currency_format($children_amount),
            'photography_text' => "(".$request->photography.")".currency_format($photography_amount),
            'tip_amount_text' => currency_format($tip_amount),
            'total_amount_text' => currency_format($total_amount_text)
        ];
    ...
    public function confirm_ride_post(Request $request){
        ...
        $dateString = session('step_one_data')['rides_date']." ".session('step_one_data')['rides_time'];
        $date = Carbon::createFromFormat('D, d M, Y h:i A', $dateString);
        $ride_date_time = $date->format('Y-m-d H:i:s');
        ...
        // Parse the provided datetime with Carbon
        $ride_date = Carbon::createFromFormat('Y-m-d H:i:s', $ride_date_time);

        // Get the current datetime
        $current_date = Carbon::now();

        // Compare current datetime with provided datetime
        // if ($current_date->greaterThan($ride_date)) {
        if (Carbon::parse($current_date)->greaterThan(Carbon::parse($ride_date))) {
            return response()->json([
                'success' => false,
                'status' => '2004',
        ...
        if (!empty(session("step_one_data")["rides_return_date"])) {
           

            $dateString = session("step_one_data")["rides_return_date"]." ".session("step_one_data")["rides_return_time"];
            $date = Carbon::createFromFormat('D, d M, Y h:i A', $dateString);
            $ride_date_time = $date->format('Y-m-d H:i:s');
        ...
        if (Auth::check() && Auth::user()->role_id != 1) {
            ...
            if (!empty($if_exist)) {
                ...
                $user_id = User::create([
                        ...
                        'whatsapp' => session("step_two_data")["watsapp"],
        ...
        if ($request->payment_method == "card") {
            ...
            if (session("step_one_data")["way"] == "two-way" && (float) session("step_two_data")["children_amount"] > 0) {
                $total_amount += (float) session("step_two_data")["children_amount"];
            }
             if (session("step_one_data")["way"] == "two-way" && (float) session("step_two_data")["tip_amount"] > 0) {
                $total_amount += (float) session("step_two_data")["tip_amount"];
            }
                ...
                'line_items' => [
                    [
                        'price_data' => [
                            ...
                            'unit_amount' => round($total_amount - (float)$copoun_amount,2) * 100
                        ],
                        'quantity' => 1,
                    ]
                ],
    ...
    public function create_ride($user,$admins,$peyment_method){
        ...
        if (empty(session("step_one_data")) || empty(session("step_two_data"))) {
        ...
        if (session("step_one_data")["ride_type_id"] == 1) {
            ...
            $ride_duration = session("step_one_data")["hours"];
        }
        ...
        $total_amount -= (float) session("step_two_data")["return_amount"];
        ...
        $ride_data = [
            ...
            'return_date' => $way != "one-way" && !empty(session("step_one_data")["rides_return_date"]) ? date('Y-m-d', strtotime(session("step_one_data")["rides_return_date"])) : null,
            'return_time' => $way != "one-way" && !empty(session("step_one_data")["rides_return_time"]) ? date('H:i:s', strtotime(session("step_one_data")["rides_return_time"])) : null,
            ...
            'currency' => !empty(\Illuminate\Support\Facades\Session::get("currency")) ? strtolower(\Illuminate\Support\Facades\Session::get("currency")) : 'aed'
        ...
        if ($way != 'one-way') {
            ...
            $total_amount = round((float) session("step_two_data")["return_amount"] + (float)session("step_two_data")["children_amount"] + (float)session("step_two_data")["tip_amount"],2);

            $return_date_time = '';

            $date = date('Y-m-d', strtotime(session("step_one_data")["rides_return_date"]));
            $time = date('H:i:s', strtotime(session("step_one_data")["rides_return_time"]));

            $dateString =session("step_one_data")["rides_return_date"]." ".session("step_one_data")["rides_return_time"];
            $date = Carbon::createFromFormat('D, d M, Y h:i A', $dateString);


            $booking_code = generateBookingNumber(session("step_one_data")["destination"], session("step_one_data")["source"], !empty( $car_info) ?  $car_info->title : '', session("step_one_data")["ride_type_id"], session("step_one_data")["way"]);
            
            $return_ride = Ride::create([
            'user_id' => $user->id,
            'parent_id' => $ride->id,
            'source' => session("step_one_data")["destination"],
            'destination' => session("step_one_data")["source"],
            'ride_type_id' => session("step_one_data")["ride_type_id"],
            'way' =>  $way === 'one-way' ? 1 : 2,
            'adults' => session("step_one_data")["adults"],
            'children' => session("step_one_data")["children"],
            ...
            'child_seats' => session("step_two_data")["child_seat"],
            'date_time' => $date->format('Y-m-d H:i:s'),
            'ride_date' => date('Y-m-d', strtotime(session("step_one_data")["rides_return_date"])),
            'ride_time' => date('H:i:s', strtotime(session("step_one_data")["rides_return_time"])),
            'car' => !empty($car_info) ? $car_info->title : '',
            'car_id' => !empty($car_info) ? $car_info->id : null,
            'payment_method' => $peyment_method,
            'price' => $total_amount,
    ...
    public function thankyou_success($ride){
      ...
      $child_ride = Ride::where('parent_id',$ride->id)->first();

    //   dd($child_ride);


      return view('front.ride.success',compact('ride','child_ride'));
    ...
    public function get_ride($id){
        ...
        return response()->json(Ride::find($id));
    }
        ...
        // }, $newFileName);
         ...
         return response()->stream(function () use ($pdf) {
                echo $pdf->output();
                    ...
                    'Content-Disposition' => 'attachment; filename="invoice_'.$ride->booking_code.'.pdf"',
                ]);
    ...
    public function get_copon_code(Request $request){
        ...
        $coupon = Coupons::where('code', $code)->where(['active' =>  true])->where('valid_until', '>=', Carbon::today())->with('ride_type')->first();

        $response = null;
        if (empty($coupon)) {

            return response()->json(['success' => false,'message' => 'This Copoun Code is not Valid!']);
        }

        if ($coupon->ride_type_id != $ride_type_id){

            $ride_type = !empty($coupon->ride_type->type) ? $coupon->ride_type->type : '';
             return response()->json(['success' => false,'message' => 'Invalid Copoun Code!, This Copoun Code is valid for only '.$ride_type.""]);
        }

        $user_info = User::where('email',session('step_two_data')["email"])->first();

        if (!empty($user_info)) {

            $copouns_used = DB::table('coupon_users')->where(['coupons_id' => $coupon->id,'user_id' => $user_info->id])->get();

            if (count($copouns_used) >= 3) {
                
                return response()->json(['success' => false,'message' => 'Sorry same coupon code can not be used more then 3 times use other account to use coupon']);
            }
        }

        $discount_amount = round(((float) $total_amount * (float) $coupon->discount_percentage) / 100,2);

        $new_total = $total_amount - (float) $discount_amount;

        $total_text_amount = currency_format($new_total);

        $discount_amount_text = currency_format($discount_amount);

        return response()->json(['success' => true,'copoun' =>$coupon,'discount_amount' => $discount_amount,'new_total' => $new_total,'total_text_amount' => $total_text_amount,'discount_amount_text' => $discount_amount_text]);
    ...
    public function store_become_partner(Request $request){
        ...
        $request->validate(
            [
                ...
                'message' => 'nullable',
                'files_upload.*' => 'required|file|mimes:jpeg,png,gif,jpg,pdf,doc,docx,txt,rtf,ppt,pptx,xls,xlsx|max:10240',
            ]
            );
            

           $partner =  BecomePartner::create(
                [
                ...
                if (!empty($request->files_upload) && count($request->files_upload) > 0) {
                    
                    foreach ($request->files_upload as $key => $value) {
                        
                        // 
                        $file_path = rand().'.'.$value->getClientOriginalExtension();
                        $extension = $value->getClientOriginalExtension();
                        $value->move(public_path('upload/partners'),$file_path);

                        BecomePartnerFiles::create(
                            [
                        ...
                        array_push( $attachements,public_path().'/upload/partners/'.$file_path);
                    }
                }
           ...
           foreach ($admins as $admin) {
              ...
              Mail::to($admin->email)->send(new ContactUsMail('Become a Partner',$admin_message,$attachements));
           }
    ...
    public function sent_contact_us(Request $request){
        $request->validate(
           ...
           foreach ($admins as $admin) {
              ...
              Mail::to($admin->email)->send(new ContactUsMail('Contact Us Mail',$admin_message,$attachements));
           }
           ...
           Mail::to($request->email)->send(new ContactUsMail('Contact Us Mail',$user_message,$attachements));
    ...
    public function reschedule_ride(Request $request,string $id){
       ...
       $request->validate(
        ...
        foreach ($admins as $admin) {
              ...
              Mail::to($admin->email)->send(new RideReschedule($admin,$ride));
    ...
    public function cancel_ride(Request $request,string $id){



        $collection = collect($request->remarks);
        $remarks = $collection->filter();


        $ride_remarks = '';

        if (!empty($remarks) && count($remarks) > 0) {

            $ride_remarks = $remarks->implode(', ');
        }


        Ride::where('id',$id)->update(['status' => 3,'cancel_remarks' =>  $ride_remarks]);
        ...
        $email = new AdminCancel($customer, $ride);
        Mail::to($ride->email)->send($email);
        ...
        foreach ($admins as $admin) {
            ...
            // $admin_message = '<p> Dear '.$admin->name."</p><p>Ride with the booking # <strong>".$ride->booking_code."</strong> has been Canceled</p><p>";
              ...
              Mail::to($admin->email)->send(new RideCancel($admin,$ride));
    ...
    public function safari_booking_post(Request $request){
        ...
        if (Auth::check() && Auth::user()->role_id != 1) {
            ...
            if (!empty($if_exist)) {
                ...
                $user_id = User::create([
                        ...
                        'whatsapp' => session("safari_step_two_data")["whatsapp"],
                        'password'=>encrypt('123455'),
                        'role_id'=> 2
                    ]);
                $user =  $user_id;
		...
		if ($request->payment_method == "card") {
            ...
            $total_amount  = (float)session("safari_step_two_data")["total_ride_amount"] - (float)$copoun_amount;
    ...
    public function safari_create_ride($user,$admins,$peyment_method){
         ...
         if (empty(session("safari_step_one_data")) || empty(session("safari_step_two_data"))) {
...
//         User::where('id',$ride->user_id)->update([
//             'mobile_number' => $ride->mobile_number 
//         ]);
//     }
// }

public function ride_tracking(){
...
public function travel_agencies_form_post(Request $request){
    $request->validate(
        [
            'first_name' => 'required',
            "last_name" => 'required',
        ...
        if (!empty($request->files_upload) && count($request->files_upload) > 0) {
            ...
            foreach ($request->files_upload as $key => $value) {
                ...
                $file_path = rand().'.'.$value->getClientOriginalExtension();
                $extension = $value->getClientOriginalExtension();
                $value->move(public_path('upload/custom-form'),$file_path);

                array_push($file_names,$file_path);

                array_push( $attachements,public_path().'/upload/custom-form/'.$file_path);
            }

        }

        TravelAgencies::create(
            [
                'type' => $request->type,
                ...
                'country_of_registration' => $request->country_of_registration,'address' => $request->address,
                'anticipated_booking' => $request->anticipated_booking,
                'proposed_date' => $request->proposed_date,
                'message' => $request->message,
                'files' => count($file_names) > 0 ? implode(",",$file_names) : ''
            ]
            );
           ...
           foreach ($admins as $admin) {
            ...
            $admin_message = '<p> Dear '.$admin->name."</p><p>".$request->type." form has been submitted with the following informations</p><p><strong>Name: ".$request->first_name." ".$request->last_name." </strong><br> <strong>Email:</strong> ".$request->email."<br> <strong>Phone: </strong>".$request->phone."<br><strong>Watsapp: </strong>".$request->watsapp."<br><strong>Business Name:</strong> ".$request->business_name."<br><strong>Company Website:</strong> ".$request->company_website."<br><strong>Position/Designation: </strong>".$request->position."<br><strong>Country of Registration:</strong> ".$request->country_of_registration."<br><strong>Anticipated Booking Volume (Monthly):</strong> ".$request->anticipated_booking."<br><strong>Proposed Start Date for Collaboration:</strong> ".$request->proposed_date."<br><strong>Head Office Address: </strong>".$request->address." <br><strong>Specific requirements or operational expectations from C2C Ride:</strong><br> ".$request->message."<br></p>";

              Mail::to($admin->email)->send(new ContactUsMail($request->type,$admin_message,$attachements));
           }

            return response()->json(['success' =>true,'message' =>"Request is successfully generated"]);
